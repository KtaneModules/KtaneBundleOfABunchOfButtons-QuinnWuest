    ...
    static partial class Ut
    {
        ...
        public static IEnumerable<T> ReduceRequiredSet<T>(IEnumerable<T> items, Func<ReduceRequiredSetState<T>, bool> test, bool breadthFirst = false, bool skipConsistencyTest = false)
        {
            ...
            while (state.AnyPartitions)
            {
                ...
                var split1 = new Range(rangeToSplit.from, mid);
                var split2 = new Range(mid + 1, rangeToSplit.to);
            ...
            public IEnumerable<T> SetToTest
            {
                get
                {
                    ...
                    if (IncludedRange != null)
                        ranges = ranges.Concat(new[] { IncludedRange.Value });
                    return ranges
        ...
        internal sealed class ReduceRequiredSetStateInternal<T> : ReduceRequiredSetState<T>
        {
            ...
            public Range LargestRange { get { return Ranges.OrderByDescending(t => t.to - t.from).First(); } }
            public Range SmallestRange { get { return Ranges.Where(r => r.from != r.to).OrderBy(t => t.to - t.from).First(); } }
